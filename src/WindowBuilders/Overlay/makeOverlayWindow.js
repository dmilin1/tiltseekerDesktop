...
const {
  GameConfigReader
} = require('./../../Utilities/GameConfigReader');
...
module.exports = async function (clientApi) {
    ...
    mouseState: state => {
      if (state == 'down') {
        ...
        this.setWindowPos = setInterval(() => {
          ...
          IOverlay.sendWindowBounds(overlayWindow.id, {
            rect: {
              x: newBounds.x,
              y: newBounds.y,
              width: width,
  ...
  overlayWindow.loadURL(`https://tiltseeker.com/?desktop=true`); // overlayWindow.loadURL(`https://tiltseeker.com/tiltseek?region=na1&summonerName=forsen&desktop=true`)
  ...
  overlayWindow.webContents.on('paint', (event, dirty, image) => {
    IOverlay.sendFrameBuffer(overlayWindow.id, image.getBitmap(), image.getSize().width, image.getSize().height);
  ...
  IOverlay.setEventCallback((event, payload) => {
    if (event == 'game.input') {
      var scaleFactor = screen.getDisplayNearestPoint(screen.getCursorScreenPoint()).scaleFactor; // payload.lparam is a 32 bit value. The HIWORD is y value, the LOWORD is x value
