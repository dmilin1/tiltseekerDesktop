...
class WindowManager {
		...
		if (this.state.child == null) {
			...
			ipc.serveNet(
				...
				() => {
					...
					// 	}
					// )
					this.state.child = execFile(
						'windowManagerHelper.exe',
						[port, this.state.timer],
						{
							cwd: __dirname,
						}
					)
	...
	verifyBounds(bounds) {
		var {topLeft, bottomRight, widthHeight } = bounds
		if (
			(
				topLeft[0] < -1000 &&
				topLeft[1] < -1000 &&
	...
	tryToUpdateBounds(newPosition) {
		...
		if (
			newPosition != null &&
			(
				this.state.currentPosition.bottomRight[0] != newPosition.bottomRight[0] ||
				this.state.currentPosition.bottomRight[1] != newPosition.bottomRight[1]
			)
		) {
			...
			if (output.includes('background')) {
				...
				if (
					(
						...
						isNaN(newPosition.widthHeight[0]) ||
						isNaN(newPosition.widthHeight[1]) ||
						isNaN(topLeft[0]) ||
						isNaN(topLeft[1]) ||
						isNaN(bottomRight[0]) ||
						isNaN(bottomRight[1])
